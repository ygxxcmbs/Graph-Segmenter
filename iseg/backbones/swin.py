...
from .cross_intra_attn import cross_intra_attn
...
class SwinTransformerBlock(tf.keras.Model):
    def __init__(
        ...
        # zhanghan
        self.use_ourmodule = use_ourmodule
        if self.use_ourmodule:
            self.cross_intra_attn = cross_intra_attn(filters, window_size=self.window_size)
    ...
    def call(self, inputs, attention_mask=None, training=None):
        ...
        #zhanghan  Add cross attention module
        # Cross-WAttn
        if self.use_ourmodule:
            attn_windows = self.cross_intra_attn(attn_windows, batchsize=tf.shape(x)[0])
