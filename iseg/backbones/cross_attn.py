...
class cross_win_attn(tf.keras.Model):

    def __init__(self,dim,window_size,name=None,):
        super().__init__(name=name)
        self.outdim = int(dim / 16)
        self.h = window_size[0]
        self.conv1 = tf.keras.layers.Conv2D(self.outdim, kernel_size=(1, 1), strides=(1, 1))
        self.W = tf.Variable(tf.initializers.GlorotUniform()([self.h * self.h * self.outdim, self.h * self.h * self.outdim]))
    ...
    def call(self, x, batchsize=1):
        #B_, L, C = x.shape  # 1568, 49, 192
        B_, L, C = tf.shape(x)[0], tf.shape(x)[1], tf.shape(x)[2]
        win_number = int(B_ / batchsize);

        # x = self.conv1(self.permute1(tf.reshape(x, [B_, self.h, self.h, C])))
        ...
        vec_dim = L * self.outdim
        x_wins = tf.reshape(tf.reshape(x1, [B_, L * self.outdim]), [batchsize, win_number, vec_dim])
        ...
        x2 = tf.matmul(tf.reshape(x2, [B_, -1]), self.W)

        # x=self.conv2(tf.reshape( tf.reshape(x,[B_, self.outdim, L])),[B_, self.outdim, self.h, self.h])
        x2 = tf.reshape(x2, [B_, self.outdim, L])
